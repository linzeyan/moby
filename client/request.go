...
func (cli *Client) doRequest(req *http.Request) (serverResponse, error) {
	...
	if err != nil {
		...
		if cli.scheme == "https" && strings.Contains(err.Error(), "bad certificate") {
			return serverResp, pkgErrors.Wrap(err, "the server probably has client authentication (--tlsverify) enabled; check your TLS client certification settings")
		}
		...
		if uErr, ok := err.(*url.Error); ok {
			if nErr, ok := uErr.Err.(*net.OpError); ok {
				if os.IsPermission(nErr.Err) {
					return serverResp, pkgErrors.Wrapf(err, "permission denied while trying to connect to the Docker daemon socket at %v", cli.host)
				}
			}
		}
		...
		return serverResp, pkgErrors.Wrap(err, "error during connect")
	}
